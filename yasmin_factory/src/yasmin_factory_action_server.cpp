...
YasminFactoryActionServer::YasminFactoryActionServer(
...
YasminFactoryActionServer::~YasminFactoryActionServer() { shutdown(); }

void YasminFactoryActionServer::shutdown() {
...
rclcpp_action::GoalResponse YasminFactoryActionServer::handle_goal(
...
void YasminFactoryActionServer::publish_feedback(
    const std::shared_ptr<GoalHandle> &goal_handle,
...
bool YasminFactoryActionServer::cancellation_requested(
    const std::shared_ptr<GoalHandle> &goal_handle) const {
  std::lock_guard<std::mutex> lock(state_mutex_);
  return cancel_requested_ || goal_handle->is_canceling();
}

void YasminFactoryActionServer::clear_active_state_machine() {
  std::lock_guard<std::mutex> lock(state_mutex_);
  active_state_machine_.reset();
  active_factory_.reset();
}

void YasminFactoryActionServer::make_available() {
  std::lock_guard<std::mutex> lock(state_mutex_);
  busy_ = false;
}
